...
from bot.services.tasks import build_tasks_for_day, format_tasks_message
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    text = (
        ...
        "Команды: /tasks /jobs /motivation /stats /plan /ping"
    )
...
async def notify_online(application) -> None:
    """Send a short 'online' message after polling starts."""
    ...
    try:
        await application.bot.send_message(
            chat_id=chat_id,
            text="✅ trabajador онлайн. Напиши /start или /ping",
        )
    except Exception as exc:
        __import__("logging").getLogger("trabajador").warning(
            "Could not send online ping to chat_id=%s: %s", chat_id, exc
        )
